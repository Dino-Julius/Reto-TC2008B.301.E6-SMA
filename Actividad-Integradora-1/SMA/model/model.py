...
class CarAgent(mesa.Agent):
    '''
    Un agente que representa un coche que se mueve entre plazas de aparcamiento en la ciudad.
    '''
...
class TrafficLightAgent(mesa.Agent):
    """
    Un agente que representa un semáforo que cambia de color entre verde y rojo.
    """
    ...
    def toggle_state(self):
        """
        Cambiar el estado del semáforo entre verde y rojo.
        """
...
class ParkingAgent(mesa.Agent):
    """
    Un agente que representa una plaza de aparcamiento.
    """
...
class BuildingAgent(mesa.Agent):
    """
    Un agente que representa un edificio.
    """
...
class RoundaboutAgent(mesa.Agent):
    """
    Un agente que representa una glorieta.
    """
...
class CityModel(mesa.Model):
    """
    La clase del modelo gestiona la cuadrícula de la ciudad con coches, plazas de aparcamiento, semáforos y edificios.
    """
    ...
    def __init__(self, num_agents, parking_positions, building_positions, roundabout_positions, traffic_lights_positions, width, height, street_directions):
        ...
        # Guardar las posiciones de los estacionamientos
        ...
        for i, pos in enumerate(traffic_lights_positions):
            ...
            traffic_light_agent = TrafficLightAgent(
                self.next_id(), self, initial_state=initial_state)
        ...
        for i in range(self.num_agents):
            start_index = self.random.choice(
                range(len(parking_positions)))  # Índice de salida
            destination_index = self.random.choice(
                range(len(parking_positions)))  # Índice de destino
            ...
            car_agent = CarAgent(self.next_id(), self,
                                 start_parking, destination_parking)
            self.schedule.add(car_agent)
            self.agent_messages.append(f"Agente {car_agent.unique_id - 203} salió de estacionamiento {
                                       start_index + 1} y se dirige a {destination_index + 1}")
    ...
    def step(self):
        """
        Un paso del modelo. Se utiliza para recopilar datos y avanzar el horario.
        """
